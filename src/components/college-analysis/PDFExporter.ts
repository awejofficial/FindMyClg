  ...
  try {
    ...
    logoImg.onload = () => {
      ...
      filteredResults.forEach((college, index) => {
        ...
        // College name (truncated if too long)
        const collegeName = college.collegeName.length > 25 ? 
          college.collegeName.substring(0, 25) + '...' : college.collegeName;
        doc.text(collegeName, 35, yPosition + 2);
        
        doc.text(college.city || 'N/A', 95, yPosition + 2);
        
        // Branch name (truncated if too long)
        const branchName = college.branch.length > 12 ? 
          college.branch.substring(0, 12) + '...' : college.branch;
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text('Generated by FindMyClg - Your DSE Admission Partner', 20, 292);
        doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 20, 297);
        doc.text(`Page ${i} of ${pageCount}`, 170, 292);
        
        // Add website URL
        doc.setTextColor(247, 68, 78);
        doc.text('findmyclg.com', 170, 297);
      }
